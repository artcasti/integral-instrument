...
<script src="js/screenfull.js"></script>
    ...
    $(function(){
      ...
      $('#btn-buscar').click(function () {
        ...
        $.get('ajax/detclientes.php', { filtro : filtro }, function(resp) {
        $('#tclientes').html(resp);
        });
      });
    });
...
<div id="wrapper">
 	...
 	<!--banner-->	
		     <div class="banner">
		           ...
		           <div class="alert alert-danger" id="mensaje_error" role="alert" hidden>Error: existen <?php if(isset($_GET['error'])){echo $_GET['error'];} ?> equipos asociados a este cliente, elimine primero los equipos y luego podrá eliminar el cliente</div> 
    ...
    <div class="justaround">
       <h5>Todos los clientes en la base</h5>
           <div class="filtro">
<!--
...
<-->
         <a href="#" style="margin-top: 0px; margin-right: 0px;" id="link-excel" ><img src="images/excel.png" alt="descargar" title="Descargar a Excel" id="btn-excel" ></a>  
         
      <label for="incompletos">Filtrar clientes incompletos</label>
       <input type="checkbox" name="incompletos" id="incompletos">
       
        <a href="form_clientes.php" class="btn btn-primary btn-xl" style="margin-top: 0px; margin-right: 0px;">
            <i class="fa fa-plus"></i><span class="hidden-sm hidden-xs">  Agregar Cliente<span/></a>
        </div>    
    </div>
...
<!--							  <td><?php echo $row[19];?></td>-->
							  ...
							  <td><?php if($row[14]) { echo 'Si';} else {echo 'No' ; }?></td>
...
<script>
$("#incompletos").change(function(){
	if($(this).is(":checked")){
	    
		var filtro = 'incompleto'; // get selected value
        $.get('ajax/tablaclientes.php', { filtro:filtro}, function(resp) {
        $('#powertable > tbody').html(resp);
       // alert(resp);
		});
	
	}else{
		var filtro = ''; // get selected value
        $.get('ajax/tablaclientes.php', { filtro:filtro}, function(resp) {
        $('#powertable > tbody').html(resp);
       // alert(resp);
	});
...
$("#btn-excel").click(function(){
	var seccion = 'clientes';
	if($("#incompletos").is(":checked")){
		var filtro = 'incompleto';
	}else{
		var filtro = '';
	}
	
	$("#link-excel").attr('href',"class/bajaexcel.php?seccion="+seccion+"&filtro="+filtro);
