...
$result = mysql_query($peticion,$conexion);
//echo "<ul>";
echo "<ul class='nav navbar-nav'>";
while ($fila = mysql_fetch_array($result)){
//echo "<li class='nivel1'><a href=".'#'." class='nivel1'>".utf8_encode($fila[1])."</a>";
echo '<li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">'.utf8_encode($fila[1]).'</a>';
...
include("conexion.php");
	...
	if (!$conexion) {die('No se puede conectar: ' . mysql_error());}
...
function registra_log($modulo,$actividad)
{
	...
	mysql_query('INSERT INTO logs (modulo,actividad, anio, mes, dia, hora, minutos, segundos, browser, fecha, usuario,ip) values (\''.$modulo.'\',\''.$actividad.'\', \''.$anio.'\',\''.$mes.'\',\''.$dia.'\',\''.$hora.'\',\''.$minutos.'\',\''.$segundos.'\',\''.$browser.'\',\''.$fecha_log.'\',\''.$usuario.'\', \''.$ip.'\')');
...
{
	$f = explode('-',$fecha);
	$texto = substr($f[2],0,2).' de '.mes($f[1]).' de '.$f[0];
	return $texto;
}
